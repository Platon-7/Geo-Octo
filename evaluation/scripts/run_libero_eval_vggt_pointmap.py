...
class GenerateConfig:
    ...
    attention_snapshot_request_pointmap_debug: bool = False  # Also capture readout tokens pre/post pointmap fusion
...
class AttentionSnapshotManager:
    ...
    def __init__(self, cfg: GenerateConfig, task_description: str, log_fn=log_message):
        ...
        self.request_pointmap_debug = bool(
            getattr(cfg, "attention_snapshot_request_pointmap_debug", False)
        )
    ...
    def build_capture_spec(self) -> dict:
        return {
            "request_tokens": True,
            "request_pointmap_debug": self.request_pointmap_debug,
        }
    ...
    def commit(
        ...
        pointmap_pre = payload.get("pointmap_readout_pre_tokens")
        pointmap_post = payload.get("pointmap_readout_post_tokens")
        pointmap_embed = payload.get("pointmap_embed_tokens")
        ...
        if pointmap_pre is not None:
            new_entries[f"{label}_readout_pre_pointmap_tokens"] = np.asarray(pointmap_pre, dtype=np.float32)
        if pointmap_post is not None:
            new_entries[f"{label}_readout_post_pointmap_tokens"] = np.asarray(pointmap_post, dtype=np.float32)
        if pointmap_embed is not None:
            new_entries[f"{label}_pointmap_embed"] = np.asarray(pointmap_embed, dtype=np.float32)
...
def run_task(
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
        ...
        if snapshot_manager is not None and snapshot_manager.captured:
    ...
    captured = snapshot_manager.captured if snapshot_manager is not None else False
    return total_episodes, total_successes, captured
