...
class AttentionSnapshotManager:
    ...
    def __init__(self, cfg: GenerateConfig, task_description: str, log_fn=logger.info):
        self.cfg = cfg
    ...
    def commit(
        ...
        # Allow future payload keys (e.g., readout tensors) without additional plumbing.
        for key, value in payload.items():
            if key in ("octo_tokens", "vggt_tokens") or value is None:
                continue
            if isinstance(value, (np.ndarray, list, tuple)):
                arr = np.asarray(value)
                new_entries[f"{label}_{key}"] = arr
