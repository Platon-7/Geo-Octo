# scripts/configs/config_no_vggt.py - Training without VGGT tokens
from ml_collections import ConfigDict
from ml_collections.config_dict import FieldReference, placeholder
from octo.utils.spec import ModuleSpec
import numpy as np

def get_config(config_string="full,multimodal"):
    mode, task = config_string.split(",")
    assert task in ["image_conditioned", "language_conditioned", "multimodal"]
    assert mode in ["full", "head_only", "head_mlp_only"]

    # Define the path to your single, unified statistics file.
    UNIFIED_STATS_PATH = "/home/pkarageorgis/geo_octo/libero_datasets/unified_stats/unified_dataset_statistics.json"

    # Use dataset WITHOUT VGGT tokens - point to your non-VGGT dataset directory
    DATASET_KWARGS_LIST = [
        {
            "name": "libero_object_no_vggt",  # Change to your non-VGGT dataset name
            "data_dir": "/path/to/your/libero_dataset_without_vggt",  # UPDATE THIS PATH
            "dataset_statistics": UNIFIED_STATS_PATH,
            "standardize_fn": ModuleSpec.create(
                "octo.data.utils.data_utils:standardize_libero_no_vggt"  # Use the new function
            ),
            "image_obs_keys": {"primary": "image_primary"},
            "proprio_obs_key": "proprio",
            "language_key": "language_instruction",
            "action_proprio_normalization_type": "normal",
            "filter_functions": [],
        },
        {
            "name": "libero_spatial_no_vggt",  # Change to your non-VGGT dataset name
            "data_dir": "/path/to/your/libero_dataset_without_vggt",  # UPDATE THIS PATH
            "dataset_statistics": UNIFIED_STATS_PATH,
            "standardize_fn": ModuleSpec.create(
                "octo.data.utils.data_utils:standardize_libero_no_vggt"  # Use the new function
            ),
            "image_obs_keys": {"primary": "image_primary"},
            "proprio_obs_key": "proprio",
            "language_key": "language_instruction",
            "action_proprio_normalization_type": "normal",
            "filter_functions": [],
        },
        {
            "name": "libero_goal_no_vggt",  # Change to your non-VGGT dataset name
            "data_dir": "/path/to/your/libero_dataset_without_vggt",  # UPDATE THIS PATH
            "dataset_statistics": UNIFIED_STATS_PATH,
            "standardize_fn": ModuleSpec.create(
                "octo.data.utils.data_utils:standardize_libero_no_vggt"  # Use the new function
            ),
            "image_obs_keys": {"primary": "image_primary"},
            "proprio_obs_key": "proprio",
            "language_key": "language_instruction",
            "action_proprio_normalization_type": "normal",
            "filter_functions": [],
        },
        {
            "name": "liber_o10_no_vggt",  # Change to your non-VGGT dataset name
            "data_dir": "/path/to/your/libero_dataset_without_vggt",  # UPDATE THIS PATH
            "dataset_statistics": UNIFIED_STATS_PATH,
            "standardize_fn": ModuleSpec.create(
                "octo.data.utils.data_utils:standardize_libero_no_vggt"  # Use the new function
            ),
            "image_obs_keys": {"primary": "image_primary"},
            "proprio_obs_key": "proprio",
            "language_key": "language_instruction",
            "action_proprio_normalization_type": "normal",
            "filter_functions": [],
        },
    ]
    
    total_steps = 150000
    stage1_steps = 5000

    if mode == "full":
       frozen_keys = None
    elif mode == "head_only":
        frozen_keys = ("octo_transformer.BlockTransformer_*",)
    else: # head_mlp_only
        frozen_keys = (
            "octo_transformer.*",
            "heads_*.map_head.probe",
            "heads_*.map_head.MultiHeadDotProductAttention_0.*",
        )

    max_steps = FieldReference(total_steps)
    window_size = FieldReference(default=2)

    config = dict(
        pretrained_path=placeholder(str),
        pretrained_step=placeholder(int),
        batch_size=4,
        shuffle_buffer_size=100,
        num_steps=total_steps,
        log_interval=100,
        eval_interval=5000,
        save_interval=5000,
        save_dir=placeholder(str),
        seed=42,
        wandb=dict(
            project="octo_no_vggt_comparison", group=placeholder(str), entity=placeholder(str)
        ),
        dataset_kwargs_list=DATASET_KWARGS_LIST,
        modality=task,
        finetuning_mode=mode,
        window_size=window_size,
        stage1_steps=stage1_steps,
        optimizer=dict(
            learning_rate=dict(
                name="cosine",
                init_value=0.0,
                peak_value=2e-6,
                warmup_steps=5000,
                decay_steps=max_steps,
                end_value=0.0,
            ),
            grad_accumulation_steps=16,
            weight_decay=0.01,
            clip_gradient=0.1,
            frozen_keys=frozen_keys,
        ),
        
        stage2_optimizer=dict(
            learning_rate=dict(
                name="cosine",
                init_value=0.0,
                peak_value=1e-6,
                warmup_steps=5000,
                decay_steps=total_steps - stage1_steps,
                end_value=0.0,
            ),
            grad_accumulation_steps=16,
            weight_decay=0.01,
            clip_gradient=1.0,
            frozen_keys=None,
            ),
        
        val_kwargs=dict(
            val_shuffle_buffer_size=50,
            num_val_batches=10,
        ),
        viz_kwargs=dict(
            eval_batch_size=4,
            trajs_for_metrics=20,
            trajs_for_viz=2,
            samples_per_state=2,
        ),
    )

    if task == "multimodal":
        task_augment_strategy="delete_task_conditioning"
        task_augment_kwargs=dict(keep_image_prob=0.5)
    else:
        task_augment_strategy=None
        task_augment_kwargs={}

    config["traj_transform_kwargs"] = dict(
        window_size=window_size,
        action_horizon=4,
        task_augment_strategy=task_augment_strategy,
        task_augment_kwargs=task_augment_kwargs,
    )

    config["frame_transform_kwargs"] = dict(
    resize_size={
        "primary": (224, 224),
    },
    image_augment_kwargs=dict(
        primary=dict(
            random_resized_crop=dict(scale=[0.8, 1.0], ratio=[0.9, 1.1]),
            random_brightness=[0.1],
            random_contrast=[0.9, 1.1],
            random_saturation=[0.9, 1.1],
            random_hue=[0.05],
            augment_order=[
                "random_resized_crop",
                "random_brightness",
                "random_contrast",
                "random_saturation",
                "random_hue",
            ],
        ),
    ),

)
    config['config_delete_keys'] = {
        "model": {
            "task_tokenizers": {
                "image": None
            }
        }
    }

    return ConfigDict(config)