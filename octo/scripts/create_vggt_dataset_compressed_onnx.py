        ...
        for episode in tqdm(ds, total=num_episodes, desc=f"Processing {self._original_builder.name}"):
            ...
            for j in range(0, num_images, batch_size):
                ...
                with tf.device('/CPU:0'):
                    ...
                    feats_small_tf = tf.image.resize(feats_tf, size=(8, 8), method='bilinear', antialias=True)  # [K*L_sel,8,8,D]
                    feats_small_flat = tf.reshape(feats_small_tf, [K, L_selected, 64, D])  # [K,L_sel,64,2048]
