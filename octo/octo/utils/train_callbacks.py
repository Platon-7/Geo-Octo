...
class VisualizationCallback(Callback):
    ...
    observation_enricher_fn: Optional[Callable[[dict], dict]] = None

    def __post_init__(self):
        # The __post_init__ is mostly correct, but we can remove the redundant line.
        # Dataclasses handle this assignment automatically.
        if self.text_processor is not None:
            self.zero_text = jax.tree_map(
                lambda x: x[0], self.text_processor.encode("")
            )
    ...
    def __call__(self, train_state: TrainState, step: int):
        wandb_metrics = {}
        # We only need one policy function definition now.
        unbatched_policy_fn = supply_rng(
            partial(
        ...
        # Wrap the unbatched function if an enricher is provided.
        if self.observation_enricher_fn:
            original_fn = unbatched_policy_fn
            def wrapped_fn(observation, tasks, policy_mode):
                enriched_obs = self.observation_enricher_fn(observation)
                return original_fn(enriched_obs, tasks, policy_mode=policy_mode)
            final_policy_fn = wrapped_fn
        else:
            final_policy_fn = unbatched_policy_fn

        modal_policy_fns = {
            mode: partial(final_policy_fn, policy_mode=mode)
            for mode in self.modes_to_evaluate
        }

        for name, visualizer in self.visualizers.items():
            # Use the correctly wrapped functions here.
...
class RolloutVisualizationCallback(Callback):
    ...
    def __call__(self, train_state: TrainState, step: int):
        ...
        if self.observation_enricher_fn:
            original_fn = base_policy_fn
            def wrapped_fn(observation, tasks, policy_mode):
                enriched_obs = self.observation_enricher_fn(observation)
                return original_fn(enriched_obs, tasks, policy_mode=policy_mode)
            final_policy_fn = wrapped_fn
        else:
            final_policy_fn = base_policy_fn
